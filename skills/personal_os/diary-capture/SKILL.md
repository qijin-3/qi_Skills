...
2. Read `{PERSONAL_OS_ROOT}/references/config.md`，取 `PATH_DIARY_PENDING`。
...
```
{PERSONAL_OS_ROOT}/state/diary_pending_{YYYY-MM-DD}.json
```

若 `{PERSONAL_OS_ROOT}/state/` 目录不存在，先创建。
...
读取 `{PERSONAL_OS_ROOT}/state/diary_session.json`（不存在则 `{}`）：
