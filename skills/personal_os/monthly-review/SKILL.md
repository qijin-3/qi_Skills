...
> **【文件路径说明】**
> - **技能参考文件**（`[技能参考]`）：与本 SKILL.md **同级**的 `references/` 目录，路径为 `monthly-review/references/xxx.md`。包括 `lark-commands.md`、`output-templates.md`、`about-me-update-rules.md`。
> - **Personal OS 数据文件**：`{PERSONAL_OS_ROOT}/content/` 或 `{PERSONAL_OS_ROOT}/state/` 下。
> - **绝对不能混淆**：`about-me-update-rules.md` 是技能参考文件，**不在** `{PERSONAL_OS_ROOT}/references/` 中。
...
**Step 0c：读基础文件**
- Read `{PERSONAL_OS_ROOT}/content/north-star.md`，取三个战略支柱名称 + 衡量标准
...
- Read `[技能参考]/about-me-update-rules.md`（Step 5 执行前必读，**此文件在技能 references/ 目录，不在 PERSONAL_OS_ROOT/references/ 下**）
...
| 3 | 上月目标打分 | 不可跳过 |
| 4 | 月度行为分析 | 无快照数据可跳过 |
| 5 | About Me 更新提案 | 无有效条目可跳过 |

---

### Step 1：收集上月数据

**飞书 CLI 语法约束** → 执行 1a–1c 前必读 `[技能参考]/lark-commands.md`。
...
**1c. 上月目标** — 读 `$TABLE_MONTHLY`（周期=上月）+ 本地 `{PERSONAL_OS_ROOT}/content/{YYYY}/{MM-1}/monthly-plan.md`，对照验收标准准备打分素材。

**1d. 月度行为数据** — 读上月所有 `{PERSONAL_OS_ROOT}/state/daily_snapshot_{date}.json`，汇总所有 `behaviors` 和 `patterns`（供 Step 4 用）。无文件则跳过，Step 4 中注明"无快照数据"。

**1e. 北极星支柱推进** — 从 1a 清单分布对照 north-star.md 三支柱，识别哪个支柱推进了、哪个被忽略（月报必须呈现）。

**1f. 日记能量（可选）** — 若上月 `{PERSONAL_OS_ROOT}/content/{YYYY}/{MM-1}/diary.md` 存在，扫描状态关键词，概述 ≤2 句。

---

### Step 2：发上月总结

按 `[技能参考]/output-templates.md` · Step 2 模板输出，填入 Step 1 数据。
...
用户确认后更新 `$TABLE_MONTHLY`（命令见 `[技能参考]/lark-commands.md`）：
- `目标完成度`：分数 ÷ 100
- `状态`：≥0.8 → 已达成；<0.8 → 已调整
...
**将月度行为分析追加到上月 `{PERSONAL_OS_ROOT}/content/{YYYY}/{MM-1}/about-me-updates.md` 末尾。**
...
Read `[技能参考]/about-me-update-rules.md` 了解各章节更新门槛，再执行。

读取上月 `{PERSONAL_OS_ROOT}/content/{YYYY}/{MM-1}/about-me-updates.md`（包含每日观察节 + Step 4 写入的月度行为总结），对照 `about-me.md` 各章节，识别满足更新门槛的稳定模式。
...
③ 同步到飞书 Wiki（命令见 `[技能参考]/lark-commands.md`）。Wiki 同步失败不影响本地文件，输出中注明即可。
