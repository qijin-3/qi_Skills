...
2. **卡路里必填**：每次记录饮食时同步估算并写入 `_卡路里`（见 `references/recording-rules.md`）
3. **安全规则优先**：`references/safety-rules.md`
4. **上下文只读本地**：`content/profile.md`、`content/{YYYY}/{MM}/monthly-plan.md`、`state/current-week.json`、脚本拉近期数据；禁止从飞书文档回读画像/月报
5. **回复简洁**：≤10 行
