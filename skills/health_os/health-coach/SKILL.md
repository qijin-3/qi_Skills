...
| 用户输入 | 动作 | 参考 |
|---------|------|------|
| 今日健康计划 / 健康提醒 | 只读 `current-week.json` 给出今日时间线；**不写 daily-log，不写 Base** | `health-daily-remind` |
...
仅当用户明确表示“已完成/刚做完”时才写入 `**运动**`。  
如果用户在问“今天做什么/计划怎么排”，该内容属于计划信息，走提醒或排期流程，不写入当日运动记录。
