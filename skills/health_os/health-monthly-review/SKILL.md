...
version: 2.2.0
description: "健康月复盘。由调度系统每月 1 号 09:00 调用，或用户说「健康月复盘」时触发。展示上月数据 → 必问 4 个目标问题 → 写 monthly-plan.md（本月目标）→ 更新 profile.md 行为模式（长期）→ 脚本同步飞书快照（只写不读）。"
---
...
**只读本地，飞书只写。** 上月数据、画像、月目标均从本地 `content/` 与脚本获取；**禁止** `lark-cli docs +fetch` 或从飞书文档/Base 回读。飞书健康画像是给用户看的镜像，月末用脚本把 `profile.md` 同步过去即可。
...
1. python3 health-system/scripts/health_analytics.py --monthly-review
   展示上月数据摘要（脚本读本地/Base 落库，不从飞书文档读）
...
7. python3 health-system/scripts/health_wiki.py --sync-profile
   （只写飞书镜像，禁止手写 lark-cli + Token；禁止从此步回读飞书）
```
